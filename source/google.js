...
GoogleNode.prototype.refreshToken = function(cb) {
    ...
    exponentialBackoff.on('ready', function() {
        ...
        .catch(function(error) {
            node.error(RED._("google.error.token-request-error", {err: error.message}));
            return exponentialBackoff.backoff();
        });
    });
...
RED.httpAdmin.get('/google-api-credentials/auth/callback', function(req, res) {
    ...
    .then(function(response) {
        ...
        .catch(function(error) {
            console.log('fetching google profile failed: ' + error);
...
GoogleAPINode.prototype.request = function(req, cb) {
    ...
    .then(function(response) {
        ...
        if (response.status >= 400) {
            return cb(RED._("google.error.httperror", {statusCode: response.status}), result);
        }
        if (result && result.status !== 'OK') {
            var error = RED._("google.error.apierror", {status: result.status});
